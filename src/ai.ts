...
function buildSystemPrompt(ctx?: ChatContext): string {
  ...
  systemParts.push(
    "回答规则：优先基于用户消息里附带的【灵犀知识库检索结果】回答，并引用采集时间。检索没覆盖用户问的，明确说\"灵犀没采集过这个话题\"，补充通识时必须标注\"以下是基于通识的推测，可能不准\"——绝不假装采集过、绝不编造细节或来源、绝不假装调用了工具。回复要简短，提供一个有意思的角度或问题，然后停。不分析用户的工作和意图。",
  );
