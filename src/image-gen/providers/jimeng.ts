...
export class JimengProvider implements ImageProvider {
  ...
  private parseOutput(stdout: string, tmpDir: string): GenerateResult {
    ...
    // 检查是否有生成的图片路径
    const imageMatch = stdout.match(/(?:result|output|image)[:\s]+(.+\.(?:jpg|jpeg|png|webp))/i);
    ...
    if (status === 'querying' && submitId) {
      throw new Error(`即梦任务提交成功（${submitId}），需要异步查询结果。请稍后重试。`);
    }

    // 尝试在临时目录找输出图片
    try {
      const files = fs.readdirSync(tmpDir).filter(f => /\.(jpg|jpeg|png|webp)$/i.test(f) && f !== 'input.jpg');
      if (files.length > 0) {
    ...
    if (submitId) {
      throw new Error(`即梦任务已提交（submit_id: ${submitId}），请用 dreamina query_result --submit_id=${submitId} 查询结果。`);
    }
 ...
 * 查询即梦异步任务结果
 */
export async function queryJimengResult(submitId: string): Promise<Buffer | null> {
  try {
    const { stdout } = await execFileAsync('dreamina', [
      'query_result',
      `--submit_id=${submitId}`,
    ], { timeout: 30000 });
    ...
    const statusMatch = stdout.match(/gen_status[:\s]+(\w+)/i);
    if (statusMatch?.[1] === 'fail') {
      const reasonMatch = stdout.match(/fail_reason[:\s]+(.+)/i);
      throw new Error(`任务失败: ${reasonMatch?.[1] || '未知原因'}`);
    }

    return null;
  } catch (err) {
    console.error('queryJimengResult failed:', err);
    throw err;
  }
}
